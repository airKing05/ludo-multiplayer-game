...
export default function PositiveY(props) {
    ...
    const YDirectionPositions = new Array(trackHeightGridCount).fill().map((_, index) => index + offset).reverse();
                        ...
                        <div className='row'>
                            {
                                XDirectionPositions.map((colIndex) => {
                                    return (<div className='col-2 border' style={{ width: `${trackGridWidth}px`,height: `${trackGridHeight}px`, fontSize:'10px', background: props.getColor(rowIndex, colIndex, '+y')}}>{colIndex}, {rowIndex}</div>)
                                })
                            }
                        </div>
                    )
